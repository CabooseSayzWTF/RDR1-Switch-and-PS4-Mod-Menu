...
void MainLRSwitches()
{
	if (ToggleSpoof13)
	{
		...
		if (PedWepz == 33)
		{
			...
			//old code works fine with emu however rather have the code on par with real hardware in the case slluxx releases his real hardware supported script loader
			//if (_STRING_COMPARE(UI_GET_STRING(SS_GET_STRING(1, consume)), "") && consume != 69 && consume != 70)stradd_s(modmsg, "--NOT DEFINED--");
			//if (!_STRING_COMPARE(UI_GET_STRING(SS_GET_STRING(1, consume)), "") && consume != 69 && consume != 70)stradd_s(modmsg, UI_GET_STRING(SS_GET_STRING(1, consume)));
			if (consume == 66)stradd_s(modmsg, "--NOT DEFINED--");//fix for real hardware bs and possibly ryujinx
